...
function showQuestions(index) {
        ...
        if (userAnswers[index]) {
            if (questions[index].numb === 1 || questions[index].numb === 3 || questions[index].numb === 4) {
                if (Array.isArray(userAnswers[index]) && userAnswers[index].includes(optionText)) {
                    optionTag.style.border = "2px solid #ffd700";
                    if (questions[index].numb === 4) {
                        selectedOption4.push(optionTag);
                    } else if (questions[index].numb === 3) {
                        selectMultiAns.push(optionTag);
                    } else if (questions[index].numb === 1) {
                        selectMultiAns.push(optionTag);
                    }
                }
            } else if (userAnswers[index] === optionText) {
                optionTag.style.border = "2px solid #ffd700";
                selectedAnswer = optionTag;
            }
        }
    ...
    // **ส่วนที่เพิ่มเข้ามาใหม่เพื่อแก้ไขบั๊ก nextBtn ไม่ทำงานหลังกลับมาคำถามเดิม:**
    if (userAnswers[index]) {
        if (questions[index].numb === 1 || questions[index].numb === 3 || questions[index].numb === 4) {
            if (userAnswers[index].length > 0) { // ตรวจสอบว่ามีการเลือกคำตอบอย่างน้อยหนึ่งข้อสำหรับคำถามหลายตัวเลือก
                nextBtn.classList.add('active');
            } else {
                nextBtn.classList.remove('active');
            }
        } else if (userAnswers[index]) { // ตรวจสอบว่ามีการเลือกคำตอบสำหรับคำถามตัวเลือกเดียว
            nextBtn.classList.add('active');
        } else {
            nextBtn.classList.remove('active');
        }
    } else {
        nextBtn.classList.remove('active'); // ถ้าไม่มีคำตอบที่เลือก ให้แน่ใจว่า nextBtn ไม่ active
    }
}
...
function selectMultipleAnswersForQ1(option, index) {
    ...
    let currentAnswers = userAnswers[index] || [];
...
nextBtn.onclick = () => {
    ...
    if (selectedAnswer || selectedOption4.length > 0 || selectMultiAns.length > 0) {
        ...
        if (questionIndex === 3 && selectMultiAns.length > 0) {
            ...
            const hasCorrectAnswer = selectMultiAns.some(ans => questions[3].answer.includes(ans.querySelector('span').textContent));
            if (hasCorrectAnswer) {
        ...
        if (corrects.includes(userAnswers[questionIndex])) {
            if (questionn.type === "Major") {
                ...
                Minor++;
                console.log({questionIndex}, ":Minor",Minor)
            }
        }
