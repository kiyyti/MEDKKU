...
function showQuestions(index) {
    ...
    questions[index].options.forEach(optionText => {
        ...
        if (userAnswers[index]) {
            if (questions[index].numb === 1 || questions[index].numb === 3 || questions[index].numb === 4) { // คำถามที่เลือกได้หลายคำตอบ
                if (Array.isArray(userAnswers[index]) && userAnswers[index].includes(optionText)) {
                    optionTag.style.border = "2px solid #ffd700"; // เลือกตัวเลือกเดิม
                    if (questions[index].numb === 4) {
                        selectedOption4.push(optionTag); // อัปเดต selectedOption4 สำหรับคำถามที่ 4
                    } else if (questions[index].numb === 3) {
                        selectMultiAns.push(optionTag); // อัปเดต selectMultiAns สำหรับคำถามที่ 3
                    } else if (questions[index].numb === 1) {
                        selectMultiAns.push(optionTag); // อัปเดต selectMultiAns สำหรับคำถามที่ 1
                    }
                }
            } else if (userAnswers[index] === optionText) { // คำถามที่เลือกได้คำตอบเดียว
                optionTag.style.border = "2px solid #ffd700"; // เลือกตัวเลือกเดิม
                selectedAnswer = optionTag; // อัปเดต selectedAnswer
            }
        }
